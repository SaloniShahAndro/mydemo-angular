...
export class RxSelectComponent extends Multilingual implements AfterContentInit, OnDestroy, OnChanges {
  ...
  setValue(isBlur: boolean = false, columnName: string = "key"): boolean {
    if (!this.autoComplete && this.currentValue.toString() && this.mainSource.length > 0) {
      var text = this.mainSource.filter(t => String(t[columnName]).toLowerCase() == this.currentValue.toString().toLowerCase())[0];
      if (text) {
